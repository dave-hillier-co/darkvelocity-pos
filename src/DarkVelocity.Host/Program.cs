...
builder.Services.AddScoped<ISearchIndexer, PostgresSearchService>();
...
var searchGroup = app.MapGroup("/api/orgs/{orgId}/search").WithTags("Search");

// GET /api/orgs/{orgId}/search/orders - Search orders
searchGroup.MapGet("/orders", async (
    Guid orgId,
    [FromQuery] string? q,
...
// GET /api/orgs/{orgId}/search/customers - Search customers
searchGroup.MapGet("/customers", async (
    Guid orgId,
    [FromQuery] string? q,
    [FromQuery] string? status,
...
// GET /api/orgs/{orgId}/search/payments - Search payments
searchGroup.MapGet("/payments", async (
    Guid orgId,
    [FromQuery] string? q,
    [FromQuery] Guid? siteId,
...
// GET /api/orgs/{orgId}/search/projection/status - Get projection status
searchGroup.MapGet("/projection/status", async (
    Guid orgId,
    IGrainFactory grainFactory) =>
{
...
// POST /api/orgs/{orgId}/search/projection/activate - Activate search projection
searchGroup.MapPost("/projection/activate", async (
    Guid orgId,
    IGrainFactory grainFactory) =>
{
    var grain = grainFactory.GetGrain<ISearchProjectionGrain>($"org:{orgId}");
